...
import { ObjectId } from "mongodb";
import { ArtistsCollection } from "../db/collections.js";
export async function getPaginatedArtists(page, limit, searchQuery) {
    const skip = (page - 1) * limit;
...
export async function getSingleArtist(artistName) {
    try {
        console.log('Service: Looking for artist:', artistName);
...
export async function getSongByArtistAndTitle(artistName, songTitle) {
    try {
        ...
        if (artist.songs && Array.isArray(artist.songs)) {
            ...
            const song = artist.songs.find((s, index) => {
        ...
        console.log('Available songs:', artist.songs?.map((s, i) => `${i}: "${s.title}"`) || []);
        return null;
    }
    catch (err) {
        console.error("getSongByArtistAndTitle error:", err);
