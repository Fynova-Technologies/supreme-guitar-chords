...
export async function getManagementToken() {
    if (!process.env.AUTH0_M2M_CLIENT_ID || !process.env.AUTH0_M2M_CLIENT_SECRET) {
        throw new Error("Missing AUTH0_M2M_CLIENT_ID or AUTH0_M2M_CLIENT_SECRET in environment variables");
    }
    ...
    if (!response.ok) {
        ...
        throw new Error(`Failed to get management token: ${JSON.stringify(errorData)}`);
    }
